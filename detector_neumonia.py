...
class ModelHandler:
    def __init__(self):
        self.model_path = 'model/conv_MLP_84.h5'
        self.model = self.load_model()

    def download_model(self):
        if not os.path.exists(self.model_path):
            url = 'https://drive.google.com/uc?id=1k-lm8w6r1I_7R8-dy9MbIQYiTxV4QiZc'
    ...
    def generate_grad_cam(self, array):
        img = ImageProcessor.preprocess(array)
        preds = self.model.predict(img)
...
class ImageProcessor:
    ...
    def read_dicom_file(path):
        try:
            img = pydicom.dcmread(path, force=True)
...
class App:
    ...
    def setup_ui(self):
        ...
        # Recognized as a class element
        self.array = None

        # Identification number to generate PDF
        self.reportID = 0
    ...
    def load_img_file(self):
        ...
        if filepath:
            if filepath.lower().endswith('.dcm'):
                self.array, img2show = ImageProcessor.read_dicom_file(filepath)
            else:
                self.array, img2show = ImageProcessor.read_image_file(filepath)
    ...
    def run_model(self):
        label, proba = self.model_handler.predict(self.array)
        heatmap = self.model_handler.generate_grad_cam(self.array)
        self.img2 = Image.fromarray(heatmap)
        self.img2 = self.img2.resize((250, 250), Image.LANCZOS)
        self.img2 = ImageTk.PhotoImage(self.img2)
        self.text_img2.image_create(END, image=self.img2)
        self.text2.insert(END, self.get_label_text(label))
